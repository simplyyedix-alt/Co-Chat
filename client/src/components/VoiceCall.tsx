  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        const callId = [uid, otherUid].sort().join('_'); callRef.current = callId; const firestore = db; if (!firestore) throw new Error('Firestore is unavailable.'); const callDoc = doc(firestore, 'calls', callId); const existing = await getDoc(callDoc); const existingActive = existing.exists() && ['ringing', 'connected'].includes(String(existing.data().status || '')); const caller = role ? role === 'caller' : (existingActive ? existing.data().callerId === uid : true)
        if (caller) {
          if (!existing.exists() || !['ringing', 'connected'].includes(String(existing.data().status || ''))) await setDoc(callDoc, { type: 'audio', callerId: uid, calleeId: otherUid, memberIds: [uid, otherUid], status: 'ringing', createdAt: serverTimestamp(), callerCandidates: [], calleeCandidates: [] })
