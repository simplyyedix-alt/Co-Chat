  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        let members = [uid, otherUid].sort(); const callId = members.join('_'); callRef.current = callId; const firestore = db; if (!firestore) throw new Error('Firestore is unavailable.'); const callDoc = doc(firestore, 'calls', callId); const existing = await getDoc(callDoc); if (existing.exists() && Array.isArray(existing.data().memberIds) && existing.data().memberIds.length === 2) members = existing.data().memberIds.map(String); const existingActive = existing.exists() && ['ringing', 'connected'].includes(String(existing.data().status || '')); const incomingForThisUser = existingActive && existing.data().status === 'ringing' && existing.data().calleeId === uid; const caller = role ? role === 'caller' : !incomingForThisUser
        if (caller) {
          await setDoc(callDoc, { type: 'audio', callerId: uid, calleeId: otherUid, memberIds: members, status: 'ringing', createdAt: serverTimestamp(), callerCandidates: [], calleeCandidates: [] })
